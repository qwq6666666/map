...
test('buildMobileOtherBrowseUI：對每個來源各自渲染一個 chip，文字含名稱與 layerCountForSource 數字', () => {
  ...
  const sourceRow = root.children[0];
  expect(sourceRow.children.length, '應該有 2 個來源 chip').toBe(2);
  expect(sourceRow.children[0].textContent, 'A 來源 chip 文字應為名稱＋圖層數').toBe('A館 2');
  expect(sourceRow.children[1].textContent, 'B 來源 chip 文字應為名稱＋圖層數').toBe('B館 1');
});
...
test('buildMobileOtherBrowseUI：初始狀態未選取任何來源，顯示提示文字，sourcesWrap 是空的', () => {
  ...
  // 這裡用 expect(!hidden).toBeTruthy() 而非 expect(...).toBe(false)，避免跟 DOM
  // stub 對「未顯式賦值屬性」的預設值（undefined）打架。
  expect(!hint.hidden, '未選取來源時應該顯示提示文字').toBeTruthy();
  expect(hint.textContent, '提示文字內容應該正確').toBe('請先選擇來源');
  expect(sourcesWrap.children.length, '未選取來源時 sourcesWrap 應該是空的').toBe(0);
});
...
test('buildMobileOtherBrowseUI：點擊 A 來源 chip，顯示 A 的 buildSourceGroup 內容、隱藏提示文字、chip 標記 active', () => {
  ...
  expect(hint.hidden, '選取來源後提示文字應該隱藏').toBe(true);
  expect(sourcesWrap.children.length, 'sourcesWrap 應該只有 1 個 buildSourceGroup 結果').toBe(1);
  expect(sourcesWrap.children[0].dataset.sourceId, '顯示的應該是 A 來源的內容').toBe('src-a');
  expect(chipA.classList.contains('active'), 'A chip 應該被標記 active').toBeTruthy();
});
...
test('buildMobileOtherBrowseUI：單選 toggle——再點一次已選中的 chip 會取消選取，回到提示文字', () => {
  ...
  expect(hint.hidden, '取消選取後應該回到顯示提示文字').toBe(false);
  expect(sourcesWrap.children.length, '取消選取後 sourcesWrap 應該清空').toBe(0);
  expect(!chipA.classList.contains('active'), 'A chip 不應該再是 active').toBeTruthy();
});
...
test('buildMobileOtherBrowseUI：切換選取——先點 A 再點 B，只顯示 B 的內容（不是疊加），A 不再 active', () => {
  ...
  expect(sourcesWrap.children.length, '切換選取後 sourcesWrap 應該只有 1 筆（B 的內容），不是疊加成 2 筆').toBe(1);
  expect(sourcesWrap.children[0].dataset.sourceId, '應該顯示 B 來源的內容').toBe('src-b');
  expect(!chipA.classList.contains('active'), 'A chip 應該不再是 active').toBeTruthy();
  expect(chipB.classList.contains('active'), 'B chip 應該變成 active').toBeTruthy();
});
...
test('buildMobileOtherBrowseUI：otherSources 為空陣列時不噴錯，chip 列是空的、顯示提示文字', () => {
  ...
  const sourceRow = root.children[0];
  const hint = root.children[1];
  expect(sourceRow.children.length, '空陣列時 chip 列應該沒有任何 chip').toBe(0);
  expect(!hint.hidden, '空陣列時仍應該顯示提示文字').toBeTruthy();
});
...
test('buildMobileOtherBrowseUI：otherSources 只有 1 筆時可以正常渲染與選取', () => {
  ...
  const sourceRow = root.children[0];
  const sourcesWrap = root.children[2];
  expect(sourceRow.children.length, '只有 1 筆來源時 chip 列應該只有 1 個 chip').toBe(1);
  sourceRow.children[0].click();
  expect(sourcesWrap.children.length, '點擊唯一的 chip 後應該正常顯示其內容').toBe(1);
  expect(sourcesWrap.children[0].dataset.sourceId).toBe('src-a');
});
...
test('全站「其他」（country===\'other\'）來源目前應為 4 個：japan／korea／ls／southeast_asia（新增/移除時要同步更新這裡）', () => {
  ...
  expect(ids.join(','), '目前「其他」分類的來源 id 應該剛好是這 4 個').toBe('japan,korea,ls,southeast_asia');
});
...
test('sidebarUI.js #categories：手機寬度下應該有 3 個 .mobile-tw-browse 容器（tw／cn／other），MOBILE_BROWSE_CONFIGS 沒有漏接 other', () => {
  ...
  expect(entries.length, '#categories 底下應該有 tw／cn／other 三個瀏覽容器').toBe(3);
});
...
test('features/multiOverlay.js #multiCategories：手機寬度下應該有 3 個 .mobile-tw-browse 容器（tw／cn／other），initMobileCountryBrowse() 的 configs 沒有漏接 other', () => {
  ...
  expect(entries.length, '#multiCategories 底下應該有 tw／cn／other 三個瀏覽容器').toBe(3);
});

...
test('neighborTiles 一般情況下回傳 8 顆鄰近圖磚，不含自己', () => {
  ...
  expect(neighbors.length, '一般情況應該是 8 顆').toBe(8);
  expect(!neighbors.some(t => t.x === 100 && t.y === 100), '不應該包含自己').toBeTruthy();
  const keys = new Set(neighbors.map(t => `${t.x}/${t.y}`));
  expect(keys.size, '8 顆座標應該彼此不重複').toBe(8);
});
...
test('neighborTiles 在世界地圖邊界（x=0）會夾回合法範圍，並排除跟自己或彼此重複的圖磚', () => {
  ...
  expect(neighbors.length, '邊界情況應該剩 5 顆（夾回後跟自己重複的被剔除）').toBe(5);
  expect(neighbors.every(t => t.x >= 0), '不該出現負的 x 座標').toBeTruthy();
  expect(!neighbors.some(t => t.x === 0 && t.y === 100), '不應該包含自己').toBeTruthy();
});

test('lonLatToTileXY 換算出的座標會被夾在合法範圍內（不會出現負值或超出 2^z-1）', () => {
  const tile = lonLatToTileXY(121.5, 25.05, 15); // 台北市中心附近
  const n = Math.pow(2, 15);
  expect(tile.x >= 0 && tile.x < n, 'x 應該在合法範圍內').toBeTruthy();
  expect(tile.y >= 0 && tile.y < n, 'y 應該在合法範圍內').toBeTruthy();
});
...
test('只有「逾時」才會自動重試一次：第一次逾時、第二次成功，最終視為有資料', async () => {
  ...
  expect(ok, '逾時後重試成功，應該視為有資料').toBeTruthy();
  expect(urlAttempts['http://x/retry-ok'], '應該總共探測了 2 次（原本 1 次逾時 + 重試 1 次）').toBe(2);
});
...
test('連續兩次都逾時，才真的判定為沒資料', async () => {
  ...
  expect(!ok, '兩次都逾時，應該視為沒資料').toBeTruthy();
  expect(urlAttempts['http://x/always-timeout'], '應該總共探測了 2 次（原本 1 次 + 逾時後重試 1 次）才判定沒資料').toBe(2);
});
...
test('伺服器明確回應沒有資料（onerror）時不重試，只探測一次', async () => {
  ...
  expect(!ok, '應該視為沒資料').toBeTruthy();
  expect(urlAttempts['http://x/always-fail'], '伺服器已經明確回應，不該重試，應該只探測 1 次').toBe(1);
});
...
test('伺服器明確回應空白小圖（onload 但過小）時不重試，只探測一次', async () => {
  ...
  expect(!ok, '過小的圖應該視為沒資料').toBeTruthy();
  expect(urlAttempts['http://x/tiny-blank'], '伺服器已經明確回應（雖然是張小圖），不該重試').toBe(1);
});
...
test('checkBatchAny：候選項目的其中一個鄰近網址成功，就算該候選項目有資料', async () => {
  ...
  expect(available.length, '應該只有一筆候選項目有資料').toBe(1);
  expect(available[0].id, '有資料的應該是 id=a').toBe('a');
});

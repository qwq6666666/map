...
- **手機版「台灣」分頁三段式瀏覽**（`src/ui/mobileTwBrowse.js`）：只有國家篩選列（`src/ui/countryFilter.js`）選到「台灣」且 `mq.matches`（<=768px）時才顯示，取代原本「來源(機構)→分類→次分類→圖層」手風琴；「中國」「其他」分頁與桌機一律維持原本手風琴不變。三段式是「大區域(全國/北部/中部/南部/東部/離島)→地區(合併同縣市的多個來源，例如 udd+taipei 都算「臺北」)→圖層」，其中第三段**不是**自己刻的扁平清單，而是重用 `sidebarUI.js` 抽出的 `buildSourceGroup(src)`（跟桌機手風琴共用同一份「建立單一來源的分類/次分類/圖層區塊」邏輯，只是依目前選中的地區篩出對應來源、各自重新 `document.createElement` 建一份獨立 DOM，不會跟桌機那份手風琴搶節點）。來源→大區域的對照表（`MACRO_REGION_MAP`）跟來源→地區標籤的字尾規則（`regionLabelForSource()`）都寫在 `mobileTwBrowse.js` 裡，新增/搬動 tw 來源時記得同步這兩份表，`tests/specs/mobile-tw-browse.test.mjs` 有全站 24 個 tw 來源的分組總數回歸測試會抓到漏改。

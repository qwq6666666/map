...
- **圖資搜尋結果面板**（`#layerSearchPanel`）手機版現在跟 `.layer-search-row` 一起被 `relocateLayerSearchRow()` 搬進 `#mobileSearchBar`，浮在頂部搜尋列正下方、自己捲動（比照 `#addressSuggest` 的浮動卡片模式：`position:absolute; top:calc(100% + 6px)`），**不再**留在側邊欄 Bottom Sheet 裡——原本因此需要的 `initLayerSearchResultAutoExpand()`（監看面板出現就強制把 Sheet 展開到 75vh，體驗上像是「一打字整個下方欄位就跳出來蓋住畫面」，已踩過的坑）已移除，不用再維護那段 MutationObserver。切到地址模式時面板靠 `body.mobile-search-mode-address` 對應規則（class 選擇器，不碰 `hidden` attribute）視覺隱藏，內容原封不動保留，切回圖資模式立即恢復顯示。
...
- **搜尋流程**（`src/ui/search.js` 的 `runImmediateSearch()`）：比對到 0 筆才退回原本的 `geocodeAddress` 流程（既有地址搜尋完全不受影響）；1 筆直接定位＋顯示卡片；多筆重用既有的 `#addressSuggest` 容器列出候選清單（**不要**另外新增獨立容器，否則手機版 `mobileLayout.js` 的 `relocateSearchBar()` 不會把它搬到頂部搜尋列，會出現「候選清單跑到看不到的地方」的 bug）。打字時的 debounce 建議清單（`input` 事件 handler，門檻常數 `ADDRESS_SUGGEST_MIN_QUERY_LENGTH`）也會並行查 `findPlaceNameCandidates()`，用 `renderMergedSuggestList()` 把地名候選（`.place-name-suggest-item`）釘在地址地理編碼建議（`.address-suggest-item`）上方一起塞進同一個 `#addressSuggest`，兩條路徑（Enter／debounce）行為一致，不會有「debounce 建議清單點了不會出地名卡」的落差。`renderPlaceNameCard()` 預設收合（加 `.collapsed`），內容照常完整渲染好只是外層 CSS 隱藏，避免選到候選後「定位列＋地名卡＋可用圖層清單」一次全部展開太長；`focusPlaceNameCard()`（identify pin「查看地名沿革」）例外，仍強制展開。

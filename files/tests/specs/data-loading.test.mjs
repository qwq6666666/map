...
test('資料載入後，19 個 WMTS 來源都在', () => {
  assertEqual(LAYER_SOURCES.length, 19, '來源數量');
});

test('資料載入後，總圖層數是 1584 筆（一筆都不能少）', () => {
  let total = 0;
  ...
  assertEqual(total, 1584, '總圖層數');
});

test('每個來源都有對應的地理範圍（REGION_EXTENTS）', () => {
  assertEqual(Object.keys(REGION_EXTENTS).length, 19, 'REGION_EXTENTS 筆數');
});

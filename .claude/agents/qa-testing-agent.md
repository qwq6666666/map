...
- 測試框架：vitest（設定檔 `vitest.config.js`，共用非同步輔助函式 `tests/helpers.mjs`）
...
1. **全域整合驗證 (Full Integration Verification)：**
   - 負責執行 `npm test` 確保所有既有測試全數通過(Green)。
...
# 驗證規範
- 執行全套測試流程以驗證程式碼健全度：
  `npm test`（等同 `vitest run`）
- 只跑特定檔案：`npx vitest run tests/specs/<test-file>.test.mjs`

...
const GUIDE_SECTIONS = [
  ...
  },
  {
    icon: '<svg class="ui-icon" aria-hidden="true" focusable="false"><use href="./assets/map-emoji-style-a-icons.svg#pin"></use></svg>',
    title: '軌跡記錄',
    body: '「⋯ 更多」選單的「記錄軌跡」會記下您走過的路線，記錄中畫面上方有狀態列顯示距離與時間，再按一次「結束記錄軌跡」即可存檔。請保持頁面在前景，螢幕鎖定或切到別的 App 時瀏覽器會暫停定位。\n\n「我的軌跡…」可顯示／隱藏、匯出 GPX／GeoJSON、存成繪圖線條、改名、刪除，也能匯入別處的 GPX／GeoJSON。\n\n<strong>軌跡只存在這台裝置的瀏覽器裡，換裝置或清資料就會消失，需要請先匯出。</strong>',
  },
  {
    ...
    title: '來源狀態／快取',
    body: '頂部「⋯ 更多」選單裡的「來源狀態／快取」可以檢查各圖資來源主機目前是否正常回應、查看最近讀取失敗的圖磚記錄，也能一鍵清除瀏覽器已下載的圖磚快取（圖層資料本身不受影響，下次瀏覽同一區域會重新下載圖磚）；按鈕旁的「已快取圖磚」會顯示目前的張數與佔用空間（MB）。圖層長時間空白、或懷疑資料沒更新時可以先來這裡看看。',
  },
  {

...
const GUIDE_SECTIONS = [
  ...
  },
  {
    ...
    title: '常見問題',
    body: '<strong>為什麼這個地方完全沒有歷史地圖可以疊？</strong> 老地圖的掃描／測繪範圍本來就有限，不是每個角落都收錄，純粹是資料沒涵蓋到，不是系統故障。\n\n<strong>圖層一直讀取失敗或空白，是不是壞掉了？</strong> 多半是來源主機暫時壅塞或逾時，可以到「⋯ 更多→來源狀態／快取」檢查主機是否正常回應，或清除快取後重新整理再試一次。\n\n<strong>分享連結給朋友，為什麼他看到的複合疊圖組合少了幾張？</strong> 自訂 WMTS／XYZ 圖層只存在您自己瀏覽器的紀錄裡，分享連結不會帶過去，對方需要自己重新加入。\n\n<strong>搜尋不到某個舊地名或古地名，怎麼辦？</strong> 地名今昔對照是精確比對資料庫既有收錄的地名，沒收錄就搜不到；可以改用一般地址搜尋定焦附近，再看看「附近歷史地名」清單有沒有相近的紀錄。\n\n<strong>重新整理頁面後，我畫的圖形或加入的自訂圖層不見了？</strong> 這些資料只存在瀏覽器本機（localStorage），沒清瀏覽器資料就會留著；換裝置、換瀏覽器或清除資料就會消失，不是雲端同步保存。\n\n<strong>這些歷史地圖可以下載或引用嗎？授權為何？</strong> 各圖層版權以其詳細資訊中標示的原始來源與版權聲明為準，學術引用請照該處標示的出處註記，不同來源授權條件可能不同。',
  },
];

...
- **`tileRenderRequestPool`（`tileLoadGuard.js` 內部，上限 `TILE_RENDER_MAX_CONCURRENCY = 4`）：第三套獨立的節流機制，套用在 `createGuardedTileLoadFunction()` 真正送出 `<img src>` 前，限制「`hist:`／`custom:` 圖層同時真正在進行中的圖磚請求數」。** 背景：實測發現 `sinica` 來源在高併發下即使是「有資料」的正常圖磚，回應也會被拖慢到 ~1 秒量級（不是只有「無資料」才慢），拖曳／縮放時這些慢請求會佔滿 OL 全域 16 個 LOADING 名額、排擠底圖。上限選 4：略低於瀏覽器對同一 host 約 6 條連線的上限，讓節流池本身才是實際瓶頸、行為可預期；雙圖比對／多重疊加模式可能同時有 2 個歷史圖層在渲染，4 留了緩衝不會讓兩者互搶卡死；底圖完全不套用這個池，OL 全域 16 個名額平時至少留 12 個給底圖。內部沒有直接 `import`／`new` `tileChecker.js` 的 `RequestPool` class：那個 class 的 `run()` 是 `async function`，就算 slot 立即可用，`await` 語意本身還是會把 `fn()` 的實際執行延到下一個 microtask 才跑，但 `attachStaleTileAbort()` 需要在同一個事件循環內同步看到 `entry.abort` 可呼叫、呼叫後 tile 狀態立即反映，所以改寫了一個介面相容（`run()`／`getStats()`）但「slot 立即可用時同步執行 `fn()`」的等價節流器（`tileLoadGuard.js` 內部的 `TileRenderPool` class）。每次嘗試（含逾時重試）都各自跟這個池要一個 slot，`entry.abort` 必須在呼叫 `run()` 之前就同步指定好，避免請求還在排隊、`abort()` 被呼叫時拿到 `null`。
...
- 測試：`tests/specs/tile-load-guard.test.mjs`（邊界外不發請求、邊界內正常載入、逾時重試、明確失敗不重試、`DEFAULT_TILE_CACHE_SIZE` 合理性、`attachStaleTileAbort()` 的 z/bbox 過期請求主動放棄、`tileRenderRequestPool` 併發上限與排隊行為、`throttle()` 的 leading/trailing 行為、`change:center` 節流版拖曳中清理等案例；fake map 的 view 額外補了 `on()`／`_triggerView()`／`_setViewState()`，供模擬拖曳中視角持續改變；`ol.proj.transformExtent` 沿用 `env-stub.mjs` 既有的 identity stub）、`tests/specs/tile-source-cache-size.test.mjs`（靜態掃描 `data.js`／`core/map.js` 原始碼字串，驗證每個 `new ol.source.*` 呼叫都帶 `cacheSize`；`data.js` 的 `ol.source.WMTS` 物件字面值內層還有一個 `{}`，不能用非貪婪正則抓「第一個 `})`」會提早截斷，要手動計數括號深度做巢狀比對），`tileXYToBbox`／`bboxIntersects` 的測試分別補在 `coordinate-transform.test.mjs`／`spatial-index.test.mjs`。
